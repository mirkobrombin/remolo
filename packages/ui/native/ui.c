...
static const nk_rune remolo_ui_terminal_glyph_ranges[] = {
...
#define REMOLO_UI_TERMINAL_INPUT_CAPACITY 16384
#define REMOLO_UI_TERMINAL_MAX_COLUMNS 240
#define REMOLO_UI_TERMINAL_MAX_ROWS 120
...
typedef struct remolo_ui {
  ...
  char terminal_input[REMOLO_UI_TERMINAL_INPUT_CAPACITY];
  ...
  struct nk_vec2 group_spacing;
  bool compact_group;
  bool closing;
  bool first_frame;
} remolo_ui;

static remolo_ui_edit_state *
remolo_ui_edit_for(remolo_ui *ui, const fdn_string *name, uint64_t capacity) {
  ...
  if (ui->edit_count == REMOLO_UI_EDIT_CAPACITY)
    return NULL;
...
static bool remolo_ui_enqueue_input(remolo_ui *ui, remolo_ui_input input) {
  ...
  if (ui->input_count == REMOLO_UI_INPUT_CAPACITY)
    return false;
...
static bool remolo_ui_inside_remote(const remolo_ui *ui, float x, float y) {
  return ui->remote_bounds_valid && x >= ui->remote_bounds.x &&
         y >= ui->remote_bounds.y &&
         x < ui->remote_bounds.x + ui->remote_bounds.w &&
...
static void remolo_ui_release_remote(remolo_ui *ui) {
  ...
  if (!ui->remote_control)
    return;
  for (key = 1; key < 256; key++) {
    if (!ui->remote_keys[key])
      continue;
    (void)remolo_ui_enqueue_input(
        ui, (remolo_ui_input){
                .kind = REMOLO_UI_INPUT_KEY, .key = key, .down = false});
    ui->remote_keys[key] = false;
  }
  for (button = 0; button < 3; button++) {
    if (!ui->remote_buttons[button])
      continue;
    (void)remolo_ui_enqueue_input(
        ui, (remolo_ui_input){.kind = REMOLO_UI_INPUT_MOUSE_BUTTON,
...
static bool remolo_ui_handle_remote_event(remolo_ui *ui,
  ...
  if (event->type == SDL_EVENT_MOUSE_WHEEL) {
    ...
    if (delta != 0) {
      (void)remolo_ui_enqueue_input(
          ui, (remolo_ui_input){.kind = REMOLO_UI_INPUT_WHEEL, .delta = delta});
    }
    return true;
  }
  if (event->type == SDL_EVENT_KEY_DOWN || event->type == SDL_EVENT_KEY_UP) {
    ...
    (void)remolo_ui_enqueue_input(ui,
                                  (remolo_ui_input){.kind = REMOLO_UI_INPUT_KEY,
                                                    .key = key,
                                                    .down = event->key.down});
    return true;
  }
...
static bool remolo_ui_terminal_append(remolo_ui *ui, const char *value,
                                      size_t length) {
  if (length > REMOLO_UI_TERMINAL_INPUT_CAPACITY - ui->terminal_input_length) {
    return false;
  }
  if (length != 0) {
...
static void remolo_ui_terminal_paste(remolo_ui *ui) {
  char *value;
  ...
  if (value == NULL)
    return;
  (void)remolo_ui_terminal_append(ui, value, SDL_strlen(value));
  SDL_free(value);
}

static const char *remolo_ui_terminal_key(SDL_Scancode scancode) {
  switch (scancode) {
  case SDL_SCANCODE_UP:
    return "\x1b[A";
  case SDL_SCANCODE_DOWN:
...
static uint8_t remolo_ui_terminal_control(SDL_Scancode scancode) {
...
static bool remolo_ui_handle_terminal_event(remolo_ui *ui,
                                            const SDL_Event *event) {
  const char *sequence;
  size_t history;
  size_t amount;
  ...
  if (event->type == SDL_EVENT_MOUSE_WHEEL && ui->terminal_bounds_valid &&
    ...
    history = remolo_terminal_history_rows(ui->terminal);
  ...
  if (event->type == SDL_EVENT_MOUSE_BUTTON_DOWN) {
    if (!remolo_ui_inside_remote(ui, event->button.x, event->button.y) &&
  ...
  if (event->type == SDL_EVENT_TEXT_INPUT) {
    if ((SDL_GetModState() & SDL_KMOD_ALT) != 0) {
      (void)remolo_ui_terminal_append(ui, "\x1b", 1);
    }
    (void)remolo_ui_terminal_append(ui, event->text.text,
  ...
  if ((event->key.mod & SDL_KMOD_SHIFT) != 0 &&
    ...
    amount = remolo_terminal_rows(ui->terminal) / 2;
    ui->terminal_scroll = ui->terminal_scroll + amount > history
                              ? history
                              : ui->terminal_scroll + amount;
    return true;
  }
  if ((event->key.mod & SDL_KMOD_SHIFT) != 0 &&
    ...
    amount = remolo_terminal_rows(ui->terminal) / 2;
    ui->terminal_scroll =
        amount >= ui->terminal_scroll ? 0 : ui->terminal_scroll - amount;
    return true;
  }
  if ((event->key.mod & SDL_KMOD_SHIFT) != 0 &&
  ...
  if ((event->key.mod & SDL_KMOD_CTRL) != 0) {
    control = remolo_ui_terminal_control(event->key.scancode);
    if (control != 0) {
      byte = (char)control;
      (void)remolo_ui_terminal_append(ui, &byte, 1);
    }
    ui->terminal_scroll = 0;
    return true;
  }
  if (event->key.scancode == SDL_SCANCODE_RETURN ||
    ...
    ui->terminal_scroll = 0;
    return remolo_ui_terminal_append(ui, "\r", 1);
  }
  if (event->key.scancode == SDL_SCANCODE_BACKSPACE) {
    ui->terminal_scroll = 0;
    return remolo_ui_terminal_append(ui, "\x7f", 1);
  }
  if (event->key.scancode == SDL_SCANCODE_TAB) {
    return remolo_ui_terminal_append(ui, "\t", 1);
  }
  if (event->key.scancode == SDL_SCANCODE_ESCAPE) {
    return remolo_ui_terminal_append(ui, "\x1b", 1);
  }
  sequence = remolo_ui_terminal_key(event->key.scancode);
  if (sequence != NULL) {
    ui->terminal_scroll = 0;
    (void)remolo_ui_terminal_append(ui, sequence, SDL_strlen(sequence));
    return true;
  }
  return true;
}
...
static void remolo_ui_edit_menu(remolo_ui *ui, struct nk_window *window,
  ...
  if (changed)
    remolo_ui_sync_edit(window, edit, buffer, capacity);
  nk_contextual_end(ui->context);
}

static void remolo_ui_apply_theme(remolo_ui *ui, bool light) {
  struct nk_color table[NK_COLOR_COUNT];
  const struct nk_color background =
      light ? nk_rgb(247, 249, 252) : nk_rgb(15, 24, 40);
  const struct nk_color panel =
      light ? nk_rgb(255, 255, 255) : nk_rgb(18, 30, 49);
  const struct nk_color raised =
      light ? nk_rgb(235, 239, 245) : nk_rgb(25, 40, 64);
  const struct nk_color border =
      light ? nk_rgb(197, 207, 221) : nk_rgb(47, 66, 94);
  const struct nk_color text =
      light ? nk_rgb(15, 24, 40) : nk_rgb(247, 249, 252);
  const struct nk_color muted =
      light ? nk_rgb(75, 91, 113) : nk_rgb(156, 172, 195);
...
static SDL_Texture *remolo_ui_logo_texture(SDL_Renderer *renderer) {
  SDL_Texture *texture;
  if (remolo_icon_rgba_len != 64U * 64U * 4U)
    return NULL;
...
static bool remolo_ui_set_window_shape(remolo_ui *ui) {
  ...
  if (!maximized) {
    for (int y = 0; y < radius; y++) {
      ...
      Uint32 *bottom = (Uint32 *)((uint8_t *)shape->pixels +
                                  (height - 1 - y) * shape->pitch);
...
int32_t remolo_ui_open(const fdn_string *title, uint64_t width, uint64_t height,
  ...
  if (!SDL_CreateWindowAndRenderer(
          window_title, (int)width, (int)height,
  ...
  terminal_config.range = remolo_ui_terminal_glyph_ranges;
  ...
  ui->terminal_font = nk_font_atlas_add_from_memory(
      atlas, (void *)remolo_dejavu_mono_ttf, remolo_dejavu_mono_ttf_len, 17.0f,
      &terminal_config);
  if (ui->regular_font == NULL || ui->heading_font == NULL ||
      ui->terminal_font == NULL) {
  ...
  ui->terminal = remolo_terminal_create(100, 32);
  if (ui->terminal == NULL) {
...
void remolo_ui_close(uint64_t *handle) {
  ...
  remolo_terminal_destroy(ui->terminal);
...
int32_t remolo_ui_begin_frame(uint64_t handle) {
  ...
  while (SDL_PollEvent(&event)) {
    ...
    if (!remolo_ui_handle_terminal_event(ui, &event) &&
        !remolo_ui_handle_remote_event(ui, &event)) {
      (void)nk_sdl_handle_event(ui->context, &event);
    }
  }
  nk_input_end(ui->context);
  ui->tooltip_length = 0;
  if (resized && !remolo_ui_set_window_shape(ui))
    return REMOLO_UI_FAILED;
  if (ui->closing)
    return 1;
  return active ? REMOLO_UI_OK : 2;
}
...
static bool remolo_ui_window_button(remolo_ui *ui,
...
void remolo_ui_titlebar(uint64_t handle, const fdn_string *title,
                        const fdn_string *subtitle) {
  remolo_ui *ui = remolo_ui_from(handle);
  int width;
  int height;
  struct nk_command_buffer *canvas;
  ...
  float title_width;
  if (ui == NULL || title == NULL || subtitle == NULL ||
  ...
  title_width = font->width(font->userdata, font->height, title->data,
                            (int)title->length);
  nk_draw_text(canvas, nk_rect(104.0f, 10.0f, title_width, 22.0f), title->data,
               (int)title->length, font, nk_rgba(0, 0, 0, 0), ui->text);
  if (subtitle->length != 0) {
    nk_draw_text(canvas,
                 nk_rect(104.0f + title_width + 16.0f, 10.0f,
                         (float)width - title_width - 252.0f, 22.0f),
                 subtitle->data, (int)subtitle->length, font,
  ...
  if ((SDL_GetWindowFlags(ui->window) & SDL_WINDOW_MAXIMIZED) == 0) {
    nk_stroke_rect(
        canvas, nk_rect(0.5f, 0.5f, (float)width - 1.0f, (float)height - 1.0f),
  ...
  if (ui->tooltip_length != 0) {
    ...
    const float measured =
        tooltip_font->width(tooltip_font->userdata, tooltip_font->height,
                            ui->tooltip, (int)ui->tooltip_length);
    struct nk_rect bounds =
        nk_rect(ui->tooltip_anchor.x + ui->tooltip_anchor.w + 8.0f,
                ui->tooltip_anchor.y + (ui->tooltip_anchor.h - 26.0f) * 0.5f,
                measured + 16.0f, 26.0f);
    if (bounds.x + bounds.w > (float)width - 8.0f) {
      bounds.x = ui->tooltip_anchor.x - bounds.w - 8.0f;
    }
    nk_fill_rect(canvas, bounds, 5.0f, ui->raised);
    nk_draw_text(canvas,
                 nk_rect(bounds.x + 8.0f, bounds.y + 4.0f, measured, 18.0f),
                 ui->tooltip, (int)ui->tooltip_length, tooltip_font,
...
bool remolo_ui_begin_group(uint64_t handle, const fdn_string *name,
                           bool scrollable) {
  remolo_ui *ui = remolo_ui_from(handle);
  char *group_name;
  ...
  if (scrollable) {
    ...
    ui->group_spacing = ui->context->style.window.spacing;
    ui->context->style.window.spacing = nk_vec2(8.0f, 2.0f);
    ui->compact_group = true;
  }
  ...
  if (scrollable && !visible) {
    ui->context->style.window.spacing = ui->group_spacing;
    ui->compact_group = false;
  }
  SDL_free(group_name);
  return visible;
}
...
void remolo_ui_separator(uint64_t handle) {
  ...
  if (nk_widget(&bounds, ui->context) == NK_WIDGET_INVALID)
    return;
  canvas = nk_window_get_canvas(ui->context);
  nk_stroke_line(canvas, bounds.x, bounds.y, bounds.x + bounds.w, bounds.y,
                 1.0f, ui->context->style.window.border_color);
}
...
static void remolo_ui_draw_icon(struct nk_command_buffer *canvas,
                                struct nk_rect bounds, uint64_t icon,
  ...
  if (icon == 0) {
    ...
    nk_stroke_line(canvas, x + 1.0f, y + 4.0f, right - 3.0f, y + 4.0f, 2.0f,
                   color);
  } else if (icon == 3) {
    const float points[] = {left,     top + 3.0f, x - 2.0f, top + 3.0f,
                            x + 1.0f, top + 6.0f, right,    top + 6.0f,
    ...
    nk_stroke_circle(canvas, nk_rect(x - 6.0f, y - 6.0f, 12.0f, 12.0f), 2.0f,
                     color);
...
bool remolo_ui_icon_button(uint64_t handle, uint64_t icon,
                           const fdn_string *label, bool selected,
                           bool enabled) {
  remolo_ui *ui = remolo_ui_from(handle);
  struct nk_rect bounds;
  struct nk_command_buffer *canvas;
  ...
  bool pressed = false;
  if (ui == NULL || label == NULL || icon > 5)
    return false;
  if (nk_widget(&bounds, ui->context) == NK_WIDGET_INVALID)
    return false;
  if (enabled) {
    pressed = nk_button_behavior(&state, bounds, &ui->context->input,
                                 NK_BUTTON_DEFAULT);
  }
  canvas = nk_window_get_canvas(ui->context);
  if (selected) {
...
void remolo_ui_heading(uint64_t handle, const fdn_string *value) {
  ...
  if (ui == NULL || value == NULL || value->length > INT32_MAX)
    return;
...
void remolo_ui_label(uint64_t handle, const fdn_string *value, uint64_t tone,
                     bool wrap) {
  remolo_ui *ui = remolo_ui_from(handle);
  ...
  selected = previous;
  if (tone == 1)
    selected = nk_rgb(156, 172, 195);
  if (tone == 2)
    selected = nk_rgb(95, 221, 122);
  if (tone == 3)
...
static struct nk_color remolo_ui_terminal_color(remolo_ui *ui, uint16_t value,
                                                bool foreground) {
  ...
  uint8_t green;
  uint8_t blue;
  if (value == REMOLO_TERMINAL_DEFAULT_COLOR) {
  ...
  green =
      (uint8_t)(((selected / 6) % 6) == 0 ? 0 : 55 + ((selected / 6) % 6) * 40);
  blue = (uint8_t)((selected % 6) == 0 ? 0 : 55 + (selected % 6) * 40);
  return nk_rgb(red, green, blue);
}

static size_t remolo_ui_terminal_rune(char *output, uint32_t rune) {
  if (rune <= 0x7f) {
    output[0] = (char)rune;
    return 1;
  }
...
static void remolo_ui_terminal_run(remolo_ui *ui,
                                   struct nk_command_buffer *canvas,
                                   const remolo_terminal_cell *cells,
                                   uint16_t begin, uint16_t end, float x,
                                   float y, float cell_width,
                                   float line_height) {
  char text[REMOLO_UI_TERMINAL_MAX_COLUMNS * 4 + 1];
  size_t length = 0;
  uint16_t column;
  ...
  bool visible = false;
  if ((attributes & REMOLO_TERMINAL_REVERSE) != 0) {
    const uint16_t swapped = foreground;
    foreground = background;
    background = swapped;
  }
  if ((attributes & REMOLO_TERMINAL_BOLD) != 0 && foreground < 8) {
    foreground = (uint16_t)(foreground + 8);
  }
  if (background != REMOLO_TERMINAL_DEFAULT_COLOR) {
    nk_fill_rect(canvas, nk_rect(x, y, width, line_height), 0.0f,
                 remolo_ui_terminal_color(ui, background, false));
  }
  for (column = begin; column < end; column++) {
    const uint32_t rune = cells[column].rune == 0 ? ' ' : cells[column].rune;
    if (rune != ' ')
      visible = true;
    length += remolo_ui_terminal_rune(text + length, rune);
  }
  if (visible) {
    struct nk_color color = remolo_ui_terminal_color(ui, foreground, true);
    if ((attributes & REMOLO_TERMINAL_DIM) != 0)
      color.a = 150;
    nk_draw_text(canvas, nk_rect(x, y, width, line_height), text, (int)length,
                 &ui->terminal_font->handle, nk_rgba(0, 0, 0, 0), color);
  }
  if ((attributes & REMOLO_TERMINAL_UNDERLINE) != 0) {
    nk_stroke_line(canvas, x, y + line_height - 2.0f, x + width,
                   y + line_height - 2.0f, 1.0f,
                   remolo_ui_terminal_color(ui, foreground, true));
  }
}

uint8_t *remolo_ui_terminal_buffer(uint64_t handle, uint64_t length,
                                   uint64_t *capacity) {
  remolo_ui *ui = remolo_ui_from(handle);
  uint8_t *storage;
...
int32_t remolo_ui_terminal_commit(uint64_t handle, uint64_t length) {
  remolo_ui *ui = remolo_ui_from(handle);
  size_t before;
  size_t after;
  if (ui == NULL || length > ui->terminal_transfer_capacity ||
      length > SIZE_MAX) {
    return REMOLO_UI_INVALID;
  }
  before = remolo_terminal_history_rows(ui->terminal);
  remolo_terminal_write(ui->terminal, ui->terminal_transfer, (size_t)length);
  after = remolo_terminal_history_rows(ui->terminal);
...
int32_t remolo_ui_terminal(uint64_t handle, float height, fdn_string *input,
                           uint64_t *columns, uint64_t *rows, bool *resized) {
  remolo_ui *ui = remolo_ui_from(handle);
  const struct nk_user_font *font;
  struct nk_command_buffer *canvas;
  struct nk_rect bounds;
  ...
  uint16_t column;
  uint16_t begin;
  if (ui == NULL || input == NULL || columns == NULL || rows == NULL ||
      resized == NULL || height < 48.0f) {
    return REMOLO_UI_INVALID;
  }
  nk_layout_row_dynamic(ui->context, height, 1);
  if (nk_widget(&bounds, ui->context) == NK_WIDGET_INVALID) {
  ...
  if (desired_columns < 2)
    desired_columns = 2;
  if (desired_columns > REMOLO_UI_TERMINAL_MAX_COLUMNS) {
    desired_columns = REMOLO_UI_TERMINAL_MAX_COLUMNS;
  }
  if (desired_rows < 2)
    desired_rows = 2;
  if (desired_rows > REMOLO_UI_TERMINAL_MAX_ROWS) {
    desired_rows = REMOLO_UI_TERMINAL_MAX_ROWS;
  }
  *resized = desired_columns != remolo_terminal_columns(ui->terminal) ||
             desired_rows != remolo_terminal_rows(ui->terminal);
  if (*resized &&
      !remolo_terminal_resize(ui->terminal, desired_columns, desired_rows)) {
    return REMOLO_UI_FAILED;
  }
  history = remolo_terminal_history_rows(ui->terminal);
  ...
  *rows = desired_rows;
  ...
  for (row = 0; row < desired_rows; row++) {
    const remolo_terminal_cell *line =
        remolo_terminal_view_row(ui->terminal, row, ui->terminal_scroll);
    if (line == NULL)
      continue;
    column = 0;
    while (column < desired_columns) {
      begin = column++;
      while (column < desired_columns &&
             line[column].foreground == line[begin].foreground &&
             line[column].background == line[begin].background &&
             line[column].attributes == line[begin].attributes) {
        column++;
      }
      remolo_ui_terminal_run(ui, canvas, line, begin, column,
                             bounds.x + 8.0f + (float)begin * cell_width,
                             bounds.y + 6.0f + (float)row * line_height,
                             cell_width, line_height);
    }
  }
  if (ui->terminal_focus && ui->terminal_scroll == 0 &&
      remolo_terminal_cursor_visible(ui->terminal)) {
    nk_stroke_rect(
        canvas,
        nk_rect(
            bounds.x + 8.0f +
                (float)remolo_terminal_cursor_column(ui->terminal) * cell_width,
            bounds.y + 6.0f +
                (float)remolo_terminal_cursor_row(ui->terminal) * line_height,
            cell_width, line_height),
        0.0f, 1.0f, ui->accent);
  }
  ...
  nk_fill_rect(
      canvas,
      nk_rect(bounds.x + bounds.w - 6.0f, bounds.y + 6.0f, 2.0f, track_height),
      1.0f, ui->panel);
  thumb_height = history == 0 ? track_height
                              : track_height * (float)desired_rows /
                                    (float)(desired_rows + history);
  if (thumb_height < 28.0f)
    thumb_height = 28.0f;
  thumb_y = bounds.y + 6.0f;
  if (history != 0) {
    thumb_y += (track_height - thumb_height) *
               (1.0f - (float)ui->terminal_scroll / (float)history);
  }
  nk_fill_rect(canvas,
               nk_rect(bounds.x + bounds.w - 7.0f, thumb_y, 4.0f, thumb_height),
               2.0f, ui->muted);
...
bool remolo_ui_button(uint64_t handle, const fdn_string *value, bool selected,
  ...
  if (primary || selected) {
    ui->context->style.button.normal =
        nk_style_item_color(nk_rgb(95, 221, 122));
    ui->context->style.button.hover =
        nk_style_item_color(nk_rgb(111, 235, 138));
    ui->context->style.button.active =
        nk_style_item_color(nk_rgb(75, 193, 101));
...
bool remolo_ui_file_entry(uint64_t handle, const fdn_string *name,
  ...
  struct nk_color foreground;
  bool pressed;
  if (ui == NULL || name == NULL || details == NULL ||
  ...
  icon_bounds = nk_rect(bounds.x + 10.0f, bounds.y + (bounds.h - 16.0f) * 0.5f,
                        18.0f, 16.0f);
  if (directory) {
...
int32_t remolo_ui_edit(uint64_t handle, const fdn_string *name,
  ...
  if (ui == NULL || name == NULL || value == NULL || result == NULL ||
      ...
      memchr(value->data, '\0', value->length) != NULL || capacity == 0 ||
  ...
  *changed = (flags & NK_EDIT_COMMITED) != 0 ||
             ...
             (result->length != 0 &&
              SDL_memcmp(result->data, value->data, result->length) != 0);
  *committed = (flags & NK_EDIT_COMMITED) != 0;
  return REMOLO_UI_OK;
}

uint8_t *remolo_ui_image_buffer(uint64_t handle, uint64_t width,
  ...
  if (ui->remote_texture == NULL || ui->remote_width != width ||
    ...
    texture =
        SDL_CreateTexture(ui->renderer, SDL_PIXELFORMAT_RGBA32,
...
void remolo_ui_image(uint64_t handle) {
  ...
  float source_ratio;
  float target_ratio;
  if (ui == NULL || ui->remote_texture == NULL)
    return;
  if (nk_widget(&bounds, ui->context) == NK_WIDGET_INVALID)
    return;
  target = bounds;
...
int32_t remolo_ui_poll_desktop_input(uint64_t handle, uint64_t *kind,
                                     uint64_t *x, uint64_t *y, uint64_t *button,
                                     uint64_t *key, bool *down,
                                     int64_t *delta) {
  remolo_ui *ui = remolo_ui_from(handle);
